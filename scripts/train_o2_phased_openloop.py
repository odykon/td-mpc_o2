"""
train_o2_phased_openloop.py — Phased training with open-loop latent CEM execution.

Identical to train_o2_phased.py (same three phases, same thresholds, same W&B
conventions), except during the `o2` phase the agent does NOT replan every env
step. Instead it plans once with CEM_in_latent_open_loop, gets back the full
decoded action sequence a:0 .. a:H-1 (H = scheduled horizon), and executes all
H actions before replanning. This isolates whether training instability comes
from replanning every step in latent space, where (unlike standard CEM) there
is no warm-starting from the previous plan.
  ...
  o2      (mujoco_latent_start_steps → mujoco_train_steps):
      Open-loop latent CEM planning (plan once per H steps), TOLD (500/ep) + decoder (100/ep).
...
Usage:
    python scripts/train_o2_phased_openloop.py cfg=cfgs/exp_phased.yaml task=walker-walk seed=1
"""
...
def train(cfg):
    ...
    print('Latent CEM mode:     open-loop (plan once per H steps)')
    print('=' * 60 + '\n')
    ...
    if checkpoint_interval in (None, 'None', 'null', ''):
    ...
    for step in range(0, cfg.train_steps + cfg.episode_length, cfg.episode_length):
        ...
        action_queue = []  # planned-but-not-yet-executed actions for open-loop o2 rollout
        while not episode.done:
            if step < cfg.seed_steps:
                ...
                if not action_queue:
                    # Replan only when the previous plan's actions are exhausted —
                    # execute a:0 .. a:H-1 open-loop instead of replanning every step.
                    _, _, _, _, _, sequence, horizon = agent.CEM_in_latent_open_loop(
                        obs, step=step, sample_final_action=True
                    )
                    action_queue = [sequence[t, 0] for t in range(horizon)]
